...
class WXMLFormatter {
    getConfiguration() {
        ...
        return {
            ...
            selfClosingTags: config.get("selfClosingTags", ["image", "input", "icon"]),
            inlineTags: config.get("inlineTags", ["text", "icon"]),
    ...
    preprocessWXML(text) {
        ...
        config.selfClosingTags.forEach(tagName => {
            // 处理有属性的标签
            processed = processed.replace(new RegExp(`<${tagName}(\\s[^>]*?)><\\/${tagName}>`, 'g'), (match, attrs) => {
                return `<${tagName}${attrs} />`;
            });
            // 处理没有属性的标签
            processed = processed.replace(new RegExp(`<${tagName}><\\/${tagName}>`, 'g'), `<${tagName} />`);
            // 规范化已有的自闭合标签
            processed = processed.replace(new RegExp(`<${tagName}([^>]*?)\\/>`, 'g'), (match, attrs) => {
    ...
    preProcessBlockTagSeparation(text) {
        let processed = text;
    ...
    postprocessWXML(text) {
        ...
        const config = this.getConfiguration();
        config.selfClosingTags.forEach(tagName => {
            processed = processed.replace(new RegExp(`<${tagName}([a-zA-Z])`, 'g'), `<${tagName} $1`);
        });
        // 最后强制处理块级标签的换行问题
        processed = this.finalBlockTagFix(processed);
        return processed;
    }
    customWXMLFormat(text) {
        const config = this.getConfiguration();
        // 1. 先处理多属性标签
    ...
    finalIndentFix(text, indentSize) {
    ...
    parseAndRebuild(text, indentSize) {
        // 移除多余的空白，但保留必要的换行
    ...
    simpleIndentCalculation(text, indentSize) {
        const lines = text.split('\n');
        const result = [];
        let depth = 0;
        let inMultiLineTag = false;
    ...
    formatWXMLAttributes(text) {
        ...
        let formatted = text;
        // 确保wx:属性的格式正确
        formatted = formatted.replace(/(wx:|bind:|catch:|capture-bind:|capture-catch:)(\w+)(\s*=\s*)(['"])/g, "$1$2=$4");
        // 格式化条件渲染 - 支持更复杂的表达式
        formatted = formatted.replace(/wx:if\s*=\s*(['"])\s*\{\{([^}]+(?:\{[^}]*\}[^}]*)*)\}\}\s*\1/g, 'wx:if="{{$2}}"');
        formatted = formatted.replace(/wx:elif\s*=\s*(['"])\s*\{\{([^}]+(?:\{[^}]*\}[^}]*)*)\}\}\s*\1/g, 'wx:elif="{{$2}}"');
        // 格式化循环 - 支持更复杂的表达式
        formatted = formatted.replace(/wx:for\s*=\s*(['"])\s*\{\{([^}]+(?:\{[^}]*\}[^}]*)*)\}\}\s*\1/g, 'wx:for="{{$2}}"');
        // 格式化其他wx:属性
        formatted = formatted.replace(/wx:for-item\s*=\s*(['"])([^'"]+)\1/g, 'wx:for-item="$2"');
        formatted = formatted.replace(/wx:for-index\s*=\s*(['"])([^'"]+)\1/g, 'wx:for-index="$2"');
        formatted = formatted.replace(/wx:key\s*=\s*(['"])([^'"]+)\1/g, 'wx:key="$2"');
        // 格式化事件绑定属性
        formatted = formatted.replace(/(bind:|catch:|capture-bind:|capture-catch:)(\w+)\s*=\s*(['"])([^'"]+)\3/g, '$1$2="$4"');
        // 格式化data-*属性
        formatted = formatted.replace(/data-(\w+)\s*=\s*(['"])\s*\{\{([^}]+(?:\{[^}]*\}[^}]*)*)\}\}\s*\2/g, 'data-$1="{{$3}}"');
        return formatted;
    }
    forceFixFormatting(text) {
        const config = this.getConfiguration();
        let formatted = text;
        // 1. 处理自闭合标签
    ...
    completeReformat(text, indentSize) {
        // 简单的标签分离和缩进修复
        let result = text;
        // 1. 确保块级标签分离
    ...
    recalculateIndentation(text, indentSize) {
        const lines = text.split('\n');
        const result = [];
        const indent = ' '.repeat(indentSize);
        // 首先，找出所有属性行并标记它们
    ...
    findTagDepthForAttribute(lines, attributeLineIndex, attributeLines) {
        // 向前查找，找到这个属性所属的标签
    ...
    isAttributeLineNew(trimmed, lines, currentIndex) {
        // 属性行的特征：
        // 1. 包含 = 符号
        // 2. 不以 < 开始（不是标签）
        // 3. 前面有一个多行标签的开始
    ...
    forceFixIndentation(text, indentSize) {
        const lines = text.split('\n');
        const result = [];
        const indent = ' '.repeat(indentSize);
        // 先计算每一行的正确深度
    ...
    formatTextTags(text) {
        ...
        config.inlineTags.forEach(tagName => {
            text = text.replace(new RegExp(`(\\s*)<${tagName}([^>]*)>\\s*([^<]*?)\\s*<\\/${tagName}>`, 'g'), (match, indent, attrs, content) => {
                // 保持原有缩进，但标签内容不换行
                const trimmedContent = content.trim();
    ...
    formatSelfClosingTags(text) {
        ...
        config.selfClosingTags.forEach(tagName => {
            // 将成对标签转换为自闭合标签
            formatted = formatted.replace(new RegExp(`<${tagName}([^>]*?)>\\s*<\\/${tagName}>`, 'g'), (match, attrs) => {
                const trimmedAttrs = attrs.trim();
                return trimmedAttrs ? `<${tagName} ${trimmedAttrs} />` : `<${tagName} />`;
            });
            // 规范化已有的自闭合标签格式
            formatted = formatted.replace(new RegExp(`<${tagName}([^>]*?)\\/>`, 'g'), (match, attrs) => {
                const trimmedAttrs = attrs.trim();
                return trimmedAttrs ? `<${tagName} ${trimmedAttrs} />` : `<${tagName} />`;
            });
        });
    ...
    formatMultiLineTagPreprocess(tagName, attrs, isSelfClosing = false) {
        const config = this.getConfiguration();
    ...
    simpleIndentFix(text, indentSize) {
        const lines = text.split('\n');
        const result = [];
        let depth = 0;
        const indent = ' '.repeat(indentSize);
    ...
    separateAllBlockTags(text, blockTags) {
        let formatted = text;
        ...
        while (formatted !== previousFormatted && iterations < maxIterations) {
            ...
            formatted = formatted.replace(/<\/([\w-]+)><([\w-]+)/g, (match, endTag, startTag) => {
                if (blockTags.includes(endTag) || blockTags.includes(startTag)) {
                    return `</${endTag}>\n<${startTag}`;
                }
                return match;
            ...
            // 2. 处理开始标签后紧跟其他标签的情况
            formatted = formatted.replace(/>(\s*)<([\w-]+)/g, (match, whitespace, nextTag) => {
            ...
            for (const line of lines) {
                ...
                // 继续处理行内可能存在的相邻标签
                processedLine = processedLine.replace(/<\/([\w-]+)><([\w-]+)/g, (match, endTag, startTag) => {
                    if (blockTags.includes(endTag) || blockTags.includes(startTag)) {
                        return `</${endTag}>\n<${startTag}`;
                    }
                    return match;
    ...
    ensureProperTagSeparation(text, blockTags, inlineTags) {
        let formatted = text;
        // 1. 处理开始标签后紧跟其他标签的情况
        formatted = formatted.replace(/>(\s*)<([\w-]+)/g, (match, whitespace, nextTag) => {
        ...
        // 2. 处理结束标签后紧跟开始标签的情况
        formatted = formatted.replace(/<\/([\w-]+)>(\s*)<([\w-]+)/g, (match, endTag, whitespace, startTag) => {
        ...
        while (formatted !== previousFormatted && iterations < maxIterations) {
            ...
            for (const line of lines) {
                ...
                // 处理开始标签后紧跟其他开始标签
                processedLine = processedLine.replace(/>(\s*)<([\w-]+)/g, (match, whitespace, nextTag) => {
                    if (blockTags.includes(nextTag) && !whitespace.includes('\n')) {
                        return `>\n<${nextTag}`;
                    }
                    return match;
    ...
    separateAdjacentBlockTags(text, blockTags) {
        let formatted = text;
    ...
    analyzeLineForIndentation(line) {
    ...
    sortTagAttributes(text) {
        ...
        return text.replace(/<([\w-]+)([^>]+)>/g, (match, tagName, attributesStr) => {
